...
const FoodDrinksCategory = () => {
  ...
  const onSubmit = async e => {
    ...
    try {
      const res = await axios.post('/sub_food_cat_upload', formData, {
        headers: {
  ...
  return (
    <React.Fragment>
      <form className="main-section-form" name="form" onSubmit={onSubmit}>
        <div>
          <label htmlFor="cat_name">Food and Drinks Category Title</label>
          <input id="cat_name" type='text' value={title} onChange={onTextChange} />
        </div>
        <div>
          <label htmlFor="cat_img">Food and Drinks Category Image</label>
...
export default FoodDrinksCategory;