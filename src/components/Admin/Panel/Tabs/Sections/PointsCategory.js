...
const PointsCategory = () => {
  ...
  const onSubmit = async e => {
    ...
    try {
      const res = await axios.post('/sub_point_cat_upload', formData, {
        headers: {
  ...
  return (
    <React.Fragment>
      <form className="main-section-form" name="form" onSubmit={onSubmit}>
        <div>
          <label htmlFor="cat_name">Points of Interest Title</label>
          <input id="cat_name" type='text' value={title} onChange={onTextChange} />
        </div>
        <div>
          <label htmlFor="cat_img">Points of Interest Image</label>
