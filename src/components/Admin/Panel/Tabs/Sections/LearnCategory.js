...
const LearnCategory = () => {
  ...
  const onSubmit = async e => {
    ...
    try {
      const res = await axios.post('/sub_learn_cat_upload', formData, {
        headers: {
  ...
  return (
    <React.Fragment>
      <form className="main-section-form" name="form" onSubmit={onSubmit}>
        <div>
          <label htmlFor="cat_name">Sport Category Title</label>
          <input id="cat_name" type='text' value={title} onChange={onTextChange} />
        </div>
        <div>
          <label htmlFor="cat_img">Sport Category Image</label>
